...
export default function LoginPage() {
  ...
  const onSubmit = async (values: LoginFormValues) => {
    ...
    try {
      ...
      if (error) {
        toast.error(error.message || 'Unable to sign in. Please verify credentials.')
        return
      }

      toast.success('Welcome back, fellow reader!')
      
      // Fetch user profile status to see if they completed onboarding
  ...
  return (
    <div className="relative min-h-[85vh] flex items-center justify-center bg-cream px-6 py-12">
      ...
      <div className="card-cozy w-full max-w-md bg-white/80 backdrop-blur-md space-y-8 z-10 p-8">
        {/* Header */}
        <div className="text-center space-y-2">
          ...
          <h2 className="font-playfair text-3xl font-bold text-warm-brown">Welcome Back</h2>
          <p className="text-sm text-navy/70">
            Pull up a chair, pour a warm drink, and sign in.
          </p>
        </div>
        ...
        <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                {...register('email')}
                className="w-full pl-11 pr-4 py-3.5 rounded-2xl border border-sage/30 bg-cream/30 focus:bg-white text-navy placeholder:text-navy/40 text-sm font-semibold transition-all focus:outline-none"
              />
            </div>
            {errors.email && (
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <Link href="#" className="text-xs font-bold text-coral hover:underline">
                Forgot?
              </Link>
            </div>
            <div className="relative">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="btn-cozy btn-cozy-primary w-full py-4 flex items-center justify-center gap-2 font-bold tracking-wide"
          >
            {isLoading ? 'Entering the room...' : 'Enter the Club'}
          </button>
        </form>
